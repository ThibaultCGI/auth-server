package io.github.tbondetti.authserver.core.exception;

public enum AuthServerErrorCode {
    USER_NOT_FOUND,
    USERNAME_IS_REQUIRED,
    USERNAME_IS_TOO_LONG,
    USERNAME_ALREADY_EXISTS,
    PASSWORD_IS_REQUIRED,
    PASSWORD_IS_TOO_SHORT,
    PASSWORD_IS_TOO_LONG,

    APPLICATION_NOT_FOUND,
    APPLICATION_CODE_ALREADY_EXISTS,
    APPLICATION_CODE_IS_REQUIRED,
    APPLICATION_CODE_IS_TOO_LONG,
    APPLICATION_NAME_IS_REQUIRED,
    APPLICATION_NAME_IS_TOO_LONG,
    APPLICATION_DESCRIPTION_IS_TOO_LONG,

    CODE_IS_REQUIRED,
    CODE_IS_TOO_LONG,
    DESCRIPTION_IS_TOO_LONG,

    NAME_IS_REQUIRED,
    NAME_IS_TOO_LONG,

    ROLE_NOT_FOUND,
    ROLE_CODE_ALREADY_EXISTS,
    ROLE_ALREADY_ASSIGNED,

    ACCESS_DENIED,
    INVALID_CREDENTIALS,
    ACCOUNT_DISABLED,

    CLIENT_ID_IS_REQUIRED,
    CLIENT_ID_IS_TOO_LONG,
    CLIENT_ID_ALREADY_EXISTS,
    CLIENT_ID_GENERATION_FAILED,

    CLIENT_NAME_IS_REQUIRED,
    CLIENT_NAME_IS_TOO_LONG,
    CLIENT_NAME_ALREADY_EXISTS,
    CLIENT_REDIRECT_URI_INVALID,
    CLIENT_GRANT_TYPE_INVALID,
    CLIENT_GRANT_TYPE_IS_REQUIRED,
    CLIENT_REDIRECT_URI_IS_REQUIRED,
    CLIENT_GRANT_TYPE_REFRESH_TOKEN_REQUIRES_AUTHORIZATION_CODE,

    CLIENT_SECRET_IS_REQUIRED,
    CLIENT_SECRET_IS_TOO_SHORT,
    CLIENT_SECRET_IS_TOO_LONG,

    CLIENT_NOT_FOUND,

    SCOPE_CODE_IS_REQUIRED,
    SCOPE_CODE_HAS_INVALID_CARACTER,
    SCOPE_CODE_IS_TOO_LONG,
    SCOPE_CODE_ALREADY_EXISTS_FOR_APPLICATION,
    SCOPE_NOT_FOUND,
    SCOPE_ALREADY_ASSIGNED,

    SCOPE_NAME_IS_REQUIRED,
    SCOPE_NAME_IS_TOO_LONG,

    SCOPE_DESCRIPTION_IS_TOO_LONG,

    ERREUR_TECHNIQUE,
    VALIDATION_ERROR,
}
